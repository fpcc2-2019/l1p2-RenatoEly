...
buscas %>% select(first_click,session_length,session_start_timestamp,results,num_clicks) %>%
    filter(first_click > 0 & num_clicks > 0) %>% 
    log() %>% 
    cor(method = "spearman")
```
...
```{r}
teste <- buscas %>% group_by(search_index) %>% 
    summarise(overall_length=mean(session_length))
