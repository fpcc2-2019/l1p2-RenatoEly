...
cdf_Date %>% 
    ...
    ggplot(aes(x=first_click,y=1-cumulative))+
    geom_line()+
    #scale_y_continuous(limits=c(0, 1))+
    facet_wrap(~session_start_date)
```
...
É possível observar que o grupo 'a' possui uma variação maior que o grupo 'b', bem como uma participação maior na quantidade total de zero resultados.

4-Let session length be approximately the time between the first event and the last event in a session. Choose a variable from the dataset and describe its relationship to session length. Visualize the relationship.

```{r}
buscas %>% filter(session_length < 100000 & group=="a" & session_start_date=="2016-03-04" & first_click>=1) %>% 
    ggplot(aes(y=session_length,x=first_click))+
    geom_point()
...
```{r}
teste <- buscas %>% group_by(first_click) %>% 
    summarise(overall_length=median(session_length))

pairs(teste)

cor(teste)

buscas %>% group_by(session_id) %>% 
    summarise(qtd_search_in_session=max(search_index)) %>% 
    group_by(qtd_search_in_session) %>% 
    summarise(qtd_session_in_search=n()) %>% 
    mutate(cumulative=cumsum(qtd_session_in_search)/sum(qtd_session_in_search)) %>% 
    filter(cumulative <= 0.99) %>% 
    ggplot(aes(x=qtd_search_in_session, y=cumulative))+
    geom_line()

buscas %>% group_by(session_id) %>% 
    filter(n()>1) %>% 
    summarise(vary=var(session_length)) %>% 
    filter(vary>0)


teste <- buscas %>% group_by(session_id) %>% 
    mutate(search_length=lead(session_start_timestamp,1) - session_start_timestamp)

teste[is.na(teste$search_length) & teste$search_index == 1,"search_length"] <- teste[is.na(teste$search_length) & teste$search_index == 1, "session_length"]
