...
```{r}
buscas %>% filter(session_length < 100000 & group=="a") %>% 
    ggplot(aes(y=session_length,x=session_start_timestamp))+
    geom_point()

buscas %>% filter(first_click!="NA" & session_length < 100000 & group=="a") %>% 
    select(first_click,session_length,session_start_timestamp,results,num_clicks) %>% cor(method = "spearman")
```
